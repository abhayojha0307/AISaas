...
const Community = () => {
  ...
  const imageLikeToggle=async(id)=>{
    try {
      ...
      const { data } = await axios.post(
        "http://localhost:3000/api/user/toggle-like-creation",
        {id},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return !loading ? (
    <div className="flex-1 flex  h-full flex-col gap-3 p-6">
      Creations
            ...
            >
              ...
              <div className="flex gap-1 items-center">
                ...
                onClick={()=>imageLikeToggle(creation.id)}
                  className={`h-5 min-w-5 hover:scale-110 cursor-pointer
                 ${
