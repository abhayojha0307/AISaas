...
import { Eraser, FileText, Hash, House, Image, LogOut, Scissors, SquarePen, User, Users } from "lucide-react";
    ...
    >
      ...
      <div className="w-full border-t border-gray-200 p-4 px-7 flex items-center justify-between">
        <div  onClick={openUserProfile} className="flex gap-2 items-center cursor-pointer">
            <img src={user.imageUrl} alt="User Profile" className="w-8 rounded-full"/>
            <div className="text-sm">
                <h1 className="font-medium text-gray-700">{user.fullName}</h1>
                <p className=" text-xs text-gray-500">Plan</p>
            </div>

        </div>
        <LogOut onClick={signOut} className="w-4.5 text-gray-300 hover:text-gray-700 cursor-pointer"/>
